...
// احتياطاً للمتصفحات القديمة
window.addEventListener('beforeunload', function() {
    syncGradesToFirebase();
});
...
// أضف هذه الدالة في ملف script.js
window.saveToFirebase = function() {
    const name = localStorage.getItem('active_user');
    const data = JSON.parse(localStorage.getItem('my_academic_data'));

    if (name && data) {
        // إنشاء مرجع في قاعدة البيانات باسم المستخدم أو كعملية حفظ جديدة
        const userGradesRef = ref(database, 'user_results');
        push(userGradesRef, {
            username: name,
            results: data,
            timestamp: new Date().toLocaleString()
        }).then(() => {
            alert("تم حفظ النقط في Firebase بنجاح! ✅");
        });
    }
};
